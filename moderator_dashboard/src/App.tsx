...
type AlertStatus = 'pending' | 'published' | 'rejected' | 'resolved' | 'expired'
...
const DEFAULT_SUPABASE_PUBLISHABLE_KEY = 'sb_publishable_msTXazQTkrFOHg1wLQ-x6w_UKOqP6v4'
const SUPABASE_URL =
...
function App() {
  ...
  const requestOtp = async (event: FormEvent) => {
    ...
    if (!normalizedPhone) {
      setError('Enter a phone number in E.164 format, for example +37255123456.')
      return
    }
  ...
  const reviewAlert = async (alertId: number, status: AlertStatus) => {
    ...
    try {
      ...
      const message =
        caughtError instanceof Error ? caughtError.message : 'Review action failed'
      setError(message)
    } finally {
      setBusyAlertId(null)
    }
    ...
    <main className="page-shell">
      ...
      {!session ? (
        <section className="panel token-panel">
          ...
          <p>Use the same phone OTP flow as the mobile app. Session is persisted automatically.</p>
          ...
          {otpRequested ? (
            <form onSubmit={verifyOtp} className="auth-form otp-form">
              ...
              <button type="submit" className="button secondary" disabled={authLoading}>
      ...
      {session ? (
        <section className="cards">
          {alerts.length === 0 ? (
            <div className="empty">No alerts found for the current filters.</div>
          ) : (
            alerts.map((alert) => (
              <article key={alert.id} className="panel card">
                ...
                <p className="body-text">{alert.body ?? 'No message provided.'}</p>
                <div className="meta-grid">
                  ...
                  <span>
                    ...
                    {alert.user?.username ?? `user_${alert.user_id ?? 'unknown'}`}
                  </span>
                  <span>
